...
// ============================================================
// Database 實現 - SQLite 数据库保存/加载（挑战一）
// 表结构：
...
bool Database::connect() {
    ...
    if (rc != SQLITE_OK) {
        std::cerr << "【数据库错误】無法打開数据库：" << sqlite3_errmsg(m_db) << "\n";
        m_connected = false; return false;
    }
    m_connected = true;
    std::cout << "【数据库】已連接：" << m_dbPath << "\n";
    if (!createTables()) {
        std::cerr << "【数据库错误】創建數據表失敗！\n"; return false;
    }
    return true;
}
...
bool Database::saveInventory(const Inventory& inventory, int playerId) {
    if (!m_connected) { std::cerr << "【数据库错误】未連接！\n"; return false; }
    ...
    for (const Item* item : items) {
        ...
        switch (item->getType()) {
            ...
            case ItemType::EQUIPMENT: {
                ...
                if (e) extra << e->getAttackBonus() << "|" << e->getDefenseBonus()
                            << "|" << static_cast<int>(e->getSlot()); break;
            }
        }
    ...
    std::cout << "【数据库】背包已保存（" << items.size() << " 種物品）\n";
    return true;
}
...
bool Database::loadInventory(Inventory& inventory, int playerId) {
    ...
    if (rc != SQLITE_OK) {
        std::cerr << "【数据库错误】查詢失敗：" << sqlite3_errmsg(m_db) << "\n";
        return false;
    }
    int count = 0;
    ...
    std::cout << "【数据库】背包已加载（" << count << " 種物品，金币 " << gold << "）\n";
    return true;
}
...
bool Database::saveShop(const Shop& shop) {
    ...
    std::cout << "【数据库】商店已保存（" << items.size() << " 種商品）\n";
    return true;
}
...
bool Database::savePlayerGold(int playerId, int gold) {
    if (!m_connected) return false;
    std::ostringstream sql;
    sql << "INSERT OR REPLACE INTO player_gold (player_id, gold) VALUES ("
        << playerId << ", " << gold << ");";
