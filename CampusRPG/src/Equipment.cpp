...
void Equipment::use(Character& target) {
    if (m_equipped) {
        std::cout << "\n>>> 【" << m_name << "】已裝備，無需重複裝備。\n";
        return;
    }
    std::cout << "\n>>> 装备【" << m_name << "】到【" << slotToString(m_slot) << "】樹位\n";
...
void Equipment::unequip(Character& target) {
    if (!m_equipped) {
        std::cout << "【" << m_name << "】未裝備，無需卸下。\n"; return;
    }
    std::cout << "\n>>> 卸下装备【" << m_name << "】\n";
    if (m_attackBonus > 0) { target.addAttack(-m_attackBonus); std::cout << "攻击力 -" << m_attackBonus << "\n"; }
    if (m_defenseBonus > 0) { target.addDefense(-m_defenseBonus); std::cout << "防御力 -" << m_defenseBonus << "\n"; }
