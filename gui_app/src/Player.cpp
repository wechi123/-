...
bool Player::costGold(int amount) {
...
// 文档规格：level * 100 (Lv1~10), level * 150 (Lv11~20), level * 200 (Lv21+)
static int calcExpNeed(int level) {
...
// ==================== 序列化 ====================
// 文档格式：name|level|hp|maxHp|exp|gold|atk|def|mp|maxMp|mdef|skillPoint
// 后续行：技能 / 背包 / 任务（扩展数据）

// 安全字符串转整数（替代 stoi，兼容 -fno-exceptions）
static inline int safeStoi(const std::string& s) {
    return (int)std::strtol(s.c_str(), nullptr, 10);
}

std::string Player::serialize() const {
    std::ostringstream oss;
    // 第1行：文档管道格式（前8字段） + 扩展字段
    oss << name << "|"
        << level << "|"
        << hp << "|" << maxHp << "|"
        << exp << "|" << gold << "|"
        << getAttack() << "|" << getDefense() << "|"
        << mp << "|" << maxMp << "|" << mdef << "|" << skillPoint << "\n";
    // 技能数据
    oss << skillList.size() << "\n";
    for (size_t i = 0; i < skillList.size(); ++i) {
        oss << skillList[i]->getId() << "," << skillList[i]->getLevel() << "\n";
    }
    // 背包道具
    oss << bagItems.size();
...
void Player::deserialize(const std::string& data) {
    ...
    std::getline(iss, line);
    {
        ...
        std::getline(ls, name, '|');
        // level
        std::getline(ls, token, '|'); level = safeStoi(token);
        // hp
        std::getline(ls, token, '|'); hp = safeStoi(token);
        // maxHp
        std::getline(ls, token, '|'); maxHp = safeStoi(token);
        // exp
        std::getline(ls, token, '|'); exp = safeStoi(token);
        // gold
        std::getline(ls, token, '|'); gold = safeStoi(token);
        // atk（跳过，由 recalcStats 重算）
        std::getline(ls, token, '|');
        // def
        std::getline(ls, token, '|'); def = safeStoi(token);
        // mp
        std::getline(ls, token, '|'); mp = safeStoi(token);
        // maxMp
        std::getline(ls, token, '|'); maxMp = safeStoi(token);
        // mdef
        std::getline(ls, token, '|'); mdef = safeStoi(token);
        // skillPoint
        std::getline(ls, token, '|'); skillPoint = safeStoi(token);
    }
